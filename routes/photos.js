...
// @desc    Get all photos with pagination and optional filters
router.get('/', async (req, res) => {
    try {
        const { page = 1, limit = 50, eventId, className, startNo, search } = req.query;
        ...
        if (className) {
            where.class = decodeURIComponent(className);
        }
        ...
        const { count, rows: photos } = await Photo.findAndCountAll({
            where,
            include: [
                { model: Event, as: 'event', attributes: ['id', 'name', 'date', 'status'] },
